...
export default class Login extends Component {
  ...
  getRememberMe = async () => {
    const exist = await AsyncStorage.getItem('userDetails');

    if(exist != null){
      var rememberMe = JSON.parse(exist.substring(1,exist.length-1));

      let userDetails = Object.assign({}, this.state.userDetails);    //creating copy of object
      userDetails.username = rememberMe.username;                        //updating value
      userDetails.password = rememberMe.password;
      userDetails.checkbox = rememberMe.checkbox;
      this.setState({userDetails});

      if(rememberMe.checkbox){
        this.setState({ checkbox: rememberMe.checkbox });
        this.setState({ username: rememberMe.username });
      }else{
        this.setState({ checkbox: rememberMe.checkbox });
  ...
  login = async (navigate) => {
    if(this.state.userDetails.checkbox != this.state.checkbox){
      let userDetails = Object.assign({}, this.state.userDetails);    //creating copy of object
      userDetails.checkbox = this.state.checkbox;

      newDetails = [];
      newDetails.push(userDetails);
      await AsyncStorage.setItem('userDetails', JSON.stringify(newDetails));
    }
      if(this.state.username == this.state.userDetails.username && this.state.password == this.state.userDetails.password){
        navigate('Main')
        title="Main";
      }else{
...
`}}/>
        ...
        <CheckBox checked={this.state.checkbox} color="#40c4ff" onPress = {() =>  this.setState({ checkbox: !this.state.checkbox })} />
        <Text style={styles.rememberMe} onPress = {() =>  this.setState({ checkbox: !this.state.checkbox })}>Remember Username</Text>
        </View>
